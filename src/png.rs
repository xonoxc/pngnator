...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        let png_signature: [u8; 8] = value[..8]
            .try_into()
            .map_err(|_| PngError::InvalidPngSignature)?;

        if png_signature != PNG_FILE_SIGNATURE {
            return Err(PngError::InvalidPngSignature);
        }

        let mut chunks_collec = Vec::<Chunk>::new();
        let mut offset = 8;
        loop {
            let curr_chunk =
                Chunk::try_from(&value[offset..]).map_err(|_| PngError::MalformedChunk)?;

            let curr_chunk_bytes = &curr_chunk.chunk_type().bytes();
            let chunk_len = curr_chunk.length();

            chunks_collec.push(curr_chunk);

            offset += 12 + chunk_len as usize;

            let str_chunk_type =
                std::str::from_utf8(curr_chunk_bytes).map_err(|_| PngError::MalformedChunk)?;
            if str_chunk_type == "IEND" || str_chunk_type == "LASt" {
                break;
            }
        }
...
impl Png {
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        ...
        res_vec.extend_from_slice(&self.header);
        res_vec.extend_from_slice(&byte_chunks);
...
mod tests {
    ...
    fn chunk_from_strings(chunk_type: &str, data: &str) -> Result<Chunk, PngError> {
        ...
        let chunk_type = ChunkType::from_str(chunk_type).map_err(|_| PngError::MalformedChunk)?;
